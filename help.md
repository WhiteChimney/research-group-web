...
4. 点击软件的上传并布署网页的按钮，等待一会即可在本网站查看修改效果
...
2. 编辑 `bib/personal_bib/xxx.bib` 文件(*~辛苦三分钟，造福全组人~*)
    ...
    4. 如果要添加 pdf 文件，请放置在 `bib/pdf` 路径下，格式为 `pdf_path={/bib/pdf/xxx.pdf},` 如下图右所示，文件名**不能有中文或空格**。
    5. 检查以上条目结尾是否缺少*逗号*
4. 点击软件的上传并布署网页的按钮，等待一会即可在本网站查看修改效果
